...
class Api
{
    ...
    protected const ENDPOINT_PREFIX = '/api/v1';
    const BASE_URI = 'https://api.qwinex.io/';
    const TIMEOUT = 10.0;
     ...
     *
     * No arguments - set QWINEX_API_KEY and QWINEX_API_SECRET in environment
    ...
    {
        $apiKey = getenv('QWINEX_API_KEY');
        $apiSecret = getenv('QWINEX_API_SECRET');
        if ($apiKey && $apiSecret) {
            $this->api_key = $apiKey;
    ...
    {
        $file = is_null($file) ? __DIR__ . "/php-qwinex-api-config.json" : $file;
